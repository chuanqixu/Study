...
def qnode_ansatzes(d, scale_factors):
    ...
    def qfunc(param, qubits, observable):
        ansatz_template(param, range(qubits))
        return qml.expval(observable)
    
    qnode_ideal = qml.QNode(functools.partial(qfunc, qubits=qubits, observable=H), dev_ideal)

    dev_noisy = qml.transforms.insert(noise_gate, noise_strength, position="all")(dev_ideal)

    qnode_noisy = qml.QNode(functools.partial(qfunc, qubits=qubits, observable=H), dev_noisy)
...
def extrapolation(d, scale_factors, plot=False):
    ...
    # Put your code here #
    zne_energy = np.polyfit(scale_factors, mitigated_energies, 2)[-1]
