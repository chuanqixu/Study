...
def shifts_and_coeffs():
    ...
    # below values are found in https://iopscience.iop.org/article/10.1088/1367-2630/ac2cb3
    ...
    return [np.pi/2, np.pi], [0.5, (np.sqrt(2)-1)/4]
...
def my_parameter_shift_grad(params):
    ...
    for i in range(len(params)):
        ...
        initial_params[i] += shifts[0]
        gradient[i] = circuit(initial_params)
        initial_params[i] -= 2*shifts[0]
        gradient[i] -= circuit(initial_params)
        gradient[i] *= coeffs[0]

        initial_params[i] += shifts[0] + shifts[1]
        gradient[i] -= coeffs[1] * circuit(initial_params)
        initial_params[i] -= 2*shifts[1]
        gradient[i] += coeffs[1] * circuit(initial_params)
