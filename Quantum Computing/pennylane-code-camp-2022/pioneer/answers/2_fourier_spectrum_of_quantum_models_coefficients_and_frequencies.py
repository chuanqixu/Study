...
def fourier_decomp(layers_params):
    ...
    def circuit(layers_params, x):
        ...
        for block_params in layers_params[:-1]:
            for basic_entangler_params in block_params:
                for i in range(4):
                    qml.RX(basic_entangler_params[i], wires=i)
                for i in range(4):
                    qml.CNOT(wires=[i, (i+1)%4])
            for i in range(4):
                qml.RX(x, wires=i, id="x")
        
        for basic_entangler_params in layers_params[-1]:
            for i in range(4):
                qml.RX(basic_entangler_params[i], wires=i)
            for i in range(4):
                qml.CNOT(wires=[i, (i+1)%4])
    ...
    frequencies = qml.fourier.circuit_spectrum(circuit, ["x"])(layers_params, x)['x']
    degree = int((len(frequencies)-1)/2)
    # do not set `lowpass_filter`, strange!
    coeffs_unorderd = qml.fourier.coefficients(functools.partial(circuit, layers_params), n_inputs=1, degree=degree)
