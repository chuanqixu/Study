...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_real, torch.ones(logits_real.shape[0]).type(logits_real.dtype).to(logits_real.device)) + bce_loss(logits_fake, torch.zeros(logits_fake.shape[0]).type(logits_fake.dtype).to(logits_fake.device))
...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.ones(logits_fake.shape[0]).type(logits_fake.dtype).to(logits_fake.device))
