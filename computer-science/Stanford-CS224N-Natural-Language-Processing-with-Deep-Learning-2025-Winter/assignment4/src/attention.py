...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    thetas = torch.arange(0, dim/2, dtype=torch.float)
    thetas = torch.pow(10000, -2 * thetas / dim)

    rope_cache = torch.arange(1, max_positions + 1, dtype=torch.float).view(-1, 1) @ thetas.view(1, -1)
    rope_cache = torch.stack((torch.cos(rope_cache), torch.sin(rope_cache)), dim=2)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    ### YOUR CODE HERE ###
    B, nh, T, hs = x.shape
    _, dim, _ = rope_cache.shape

    x_complex = torch.view_as_complex(x.transpose(1, 2).view(B, T, dim, 2))
    rope_cache_complex = torch.view_as_complex(rope_cache[:T, :, :]).view(1, 1, T, dim)
    rotated_x = torch.view_as_real(rope_cache_complex * x_complex).view(B, T, nh, hs).transpose(1, 2)
    ### END YOUR CODE ###
    return rotated_x
