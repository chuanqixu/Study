...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        dec_t = dec_init_state
        o_t = torch.zeros((batch_size, self.hidden_size), device=self.device)
        for Y_t in torch.split(Y, split_size_or_sections=1):
            Ybar_t = torch.cat((Y_t.view(batch_size, -1), o_t), 1)
            dec_t, o_t, e_t = self.step(Ybar_t, dec_t, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = (enc_hiddens.permute(0, 2, 1) @ alpha_t.unsqueeze(2)).squeeze(2)
