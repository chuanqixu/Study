...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i=omp_get_thread_num()*chunk_size; i<(omp_get_thread_num()+1)*chunk_size; i++)
      z[i] = x[i] + y[i];
  }
  for(int i=omp_get_num_threads(); i<ARRAY_SIZE; i++)
      z[i] = x[i] + y[i];
}
// END PART 1 EX 2
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp critical
      global_sum += sum[omp_get_thread_num()];
  }
  return global_sum;
}
