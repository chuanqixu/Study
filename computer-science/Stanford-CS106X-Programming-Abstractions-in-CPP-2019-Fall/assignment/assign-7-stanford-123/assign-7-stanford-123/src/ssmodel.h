...
private:
    ...
    // data related
    SparseGrid<Expression *> cells;
    EvaluationContext eval_context;
    SparseGrid<double> caches;

    // dependancies related
    Graph<node, arc> dependencies;

    // get the pointer of the cell, or return nullptr if there is no element
    Expression *get_cell(const std::string &cellname) const;

    // change a cell range to a vector a double
    std::shared_ptr<Vector<double>> range_to_vector(const range &ran);



    // if the expression is an identifier, add it to symbolTable in eval_context
    bool update_eval_context(Expression &exp);

    // update SSView starting from the given cell
    void update_view(std::string cellname);
};
